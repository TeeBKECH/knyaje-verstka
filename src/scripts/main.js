...
import { attachCustomScrollbar } from '@/scripts/components/scroll-bar.js'
...
import { Quiz } from '@/scripts/components/quiz.js'
...
document.addEventListener('DOMContentLoaded', (e) => {
  ...
  document
    .querySelectorAll('.gallery [data-fancybox], .gallery-mosaic [data-fancybox]')
    .forEach((el) => {
   ...
   * Quiz
   */
  const quizElement = document.querySelector('[data-component="quiz"]')
  if (quizElement) {
    const quiz = new Quiz(quizElement)
    // Можно подписаться на событие отправки формы
    quizElement.addEventListener('quiz:submit', (e) => {
      console.log('Quiz submitted:', e.detail.data)
      // Здесь можно добавить отправку на сервер, аналитику и т.д.
    })
  }
